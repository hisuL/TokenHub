export const enTranslations: Record<string, string> = {
    ...
    "团队总览": "Team Overview",
    "团队报表": "Team Reports",
    "成本归因": "Cost Attribution", "外部账单连接器": "External Billing Connectors", "新增连接器": "Add Connector", "调度": "Schedule", "最近同步": "Recent Sync", "编辑连接器": "Edit Connector", "测试连接": "Test Connection", "立即同步": "Sync Now", "停用连接器": "Disable Connector", "启用连接器": "Enable Connector", "账单": "Bills", "完成时间": "Finished", "定时": "Scheduled", "外部账单明细": "External Billing Records", "产品 / 模型": "Product / Model", "净额": "Net Amount", "编辑账单连接器": "Edit Billing Connector", "新增账单连接器": "Add Billing Connector", "同步间隔（分钟）": "Sync Interval (minutes)", "每秒请求上限": "Requests per Second", "币种": "Currency", "源时区": "Source Time Zone", "日志路径": "Log Path", "NewAPI 用户 ID": "NewAPI User ID", "每币种单位 Quota": "Quota per Currency Unit", "连接测试通过": "Connection test passed", "账单同步完成": "Billing sync completed", "连接器已启用": "Connector enabled", "连接器已停用": "Connector disabled", "连接器已保存": "Connector saved",
    ...
    "连接": "Connect",
    "高级": "Advanced",
    "Claude Code 推理兼容": "Claude Code reasoning compatibility", "配置推理强度值域、预算映射和 reasoning_content 多轮回传。": "Configure effort values, budget mapping, and multi-turn reasoning_content replay.", "Provider 资源推理兼容": "Provider resource reasoning compatibility", "资源级配置覆盖 Provider 配置，适合不同上游端点使用不同的推理参数规则。": "Resource settings override Provider settings when upstream endpoints need different reasoning rules.", "Anthropic 推理参数兼容": "Anthropic reasoning parameter compatibility", "Provider 默认规则，适用于 Claude Code 等 Anthropic Messages 客户端。": "Provider defaults for Anthropic Messages clients such as Claude Code.", "资源级推理参数覆盖": "Resource reasoning parameter overrides", "资源默认继承 Provider；仅当该上游端点规则不同时启用覆盖。": "Resources inherit the Provider by default. Override only when this upstream endpoint requires different rules.", "配置来源": "Configuration source", "继承 Provider": "Inherit Provider", "自定义覆盖": "Custom override", "保存后，该资源使用下面的完整规则覆盖 Provider 默认规则。": "After saving, this resource uses the complete rules below instead of the Provider defaults.", "当前使用 Provider 默认规则；保存后会清除已有资源级覆盖。": "Provider defaults are currently used. Saving clears existing resource overrides.", "继承 Provider Base URL": "Inherit Provider Base URL", "保存 Provider 资源推理兼容设置": "Save Provider resource reasoning settings", "Provider 资源推理兼容设置已保存。": "Provider resource reasoning settings saved.", "保存资源设置": "Save resource settings", "推理强度允许值": "Allowed reasoning effort values", "填写上游实际接受的 reasoning_effort 值，逗号分隔；留空表示不限制。": "Enter the reasoning_effort values accepted upstream, separated by commas. Leave blank to allow any value.", "推理强度映射": "Reasoning effort mapping", "把 Claude Code 的推理强度转换为上游值；填写 JSON 对象。": "Map Claude Code reasoning effort to upstream values. Enter a JSON object.", "不支持值处理": "Unsupported value handling", "透传原值、删除参数，或在调用上游前返回明确错误。": "Pass through, omit the parameter, or reject before calling upstream.", "推理预算映射": "Reasoning budget mapping", "按 thinking.budget_tokens 的最大 Token 数映射推理强度；* 表示兜底值。": "Map thinking.budget_tokens ceilings to reasoning effort; * is the fallback.", "回传推理内容": "Replay reasoning content", "仅在上游支持后续 assistant 消息接收 reasoning_content 时开启。": "Enable only when the upstream accepts reasoning_content in later assistant messages.", "透传": "Pass through", "删除参数": "Omit parameter", "拒绝请求": "Reject request", "推理强度映射必须是键值均为字符串的 JSON 对象。": "Reasoning effort mapping must be a JSON object with string keys and values.", "推理预算映射必须是键值均为字符串的 JSON 对象。": "Reasoning budget mapping must be a JSON object with string keys and values.",
    ...
    "先在“连接”中填写 Base URL 和 API Key，这里会加载自定义渠道的上游模型。": "Enter the Base URL and API key under Connection, and the custom Provider's upstream models load here.",
    "请先加载自定义渠道的上游模型，再选择要引入的模型。": "Load the custom Provider's upstream models first, then choose which ones to import.",
    ...
    "正在打开授权页": "Opening Authorization Page", "正在复制授权链接": "Copying Authorization Link",
    "我知道了，打开授权页": "Got It, Open Authorization", "我知道了，复制授权链接": "Got It, Copy Authorization Link",
    ...
    "生成账号授权地址失败": "Failed to generate account authorization URL", "复制授权链接失败，请允许浏览器访问剪贴板后重试。": "Failed to copy the authorization link. Allow clipboard access in your browser and try again.",
    ...
    "已从粘贴的回调结果回填账号 Token。": "Account token was filled from the pasted callback result.", "已复制 OpenAI/Codex 授权链接。请在新标签页打开链接并完成授权。": "Copied the OpenAI/Codex authorization link. Open it in a new tab and complete authorization.",
    ...
    "确认禁用": "Confirm disable", "确认启用": "Confirm enable",
    "查询 Codex 重置次数": "Query Codex reset credits", "查询 Codex 重置次数失败": "Failed to query Codex reset credits", "重置 Codex 用量窗口": "Reset Codex usage window", "重置请求已完成，正在刷新额度。": "The reset request completed. Refreshing quota now.", "该重置请求此前已完成，正在刷新额度。": "This reset request had already completed. Refreshing quota now.", "受影响窗口": "Affected windows", "重置 Codex 用量窗口失败": "Failed to reset the Codex usage window", "剩余重置次数": "Remaining reset credits", "查询中": "Loading", "最近可用次数过期": "Earliest available credit expires in", "最近可用次数过期时间": "Earliest available credit expires at", "选择要使用的重置次数": "Choose a reset credit", "重置次数": "Reset credit", "重置套餐": "Reset Plan", "Codex 额度安全操作": "Codex quota safety action", "确认重置 Codex 用量窗口": "Reset the Codex usage window?", "当前剩余重置次数": "Current reset credits", "确认后将消耗 1 次重置次数，并立即重置当前 Codex 用量窗口；不会更改账号套餐类型。": "Confirming consumes one reset credit and immediately resets the current Codex usage window. It does not change the account plan.", "已使用的重置次数无法恢复。": "Used reset credits cannot be restored.", "正在重置": "Resetting", "正在重置 Codex 套餐": "Resetting the Codex plan", "正在提交重置并刷新用量与重置次数，请勿关闭页面或重复操作。": "Submitting the reset and refreshing usage and reset credits. Do not close this page or repeat the operation.", "确认重置": "Confirm reset", "选中次数过期": "Selected credit expires in", "选中次数过期时间": "Selected credit expires at", "可用重置次数明细暂不可用、已过期或不一致，请点击“刷新用量与重置次数”重新查询。": "Reset-credit details are unavailable, expired, or inconsistent. Refresh usage and reset credits to query them again.", "实时查询 ChatGPT/Codex 套餐用量和重置时间；每 10 分钟自动刷新，也可手动刷新。": "Query live ChatGPT/Codex plan usage and reset times. Data refreshes every 10 minutes and can also be refreshed manually.", "刷新用量与重置次数": "Refresh usage and reset credits", "查询用量与重置次数": "Query usage and reset credits", "该操作已经提交。为避免重复消耗，请保留本弹窗并使用同一次操作重试，直到获得确定结果。": "This operation was submitted. To avoid consuming another credit, keep this dialog open and retry the same operation until its result is known.", "选中的重置次数已经过期，请取消并刷新额度。": "The selected reset credit has expired. Cancel and refresh quota.", "无法保存本次重置的安全状态，请检查浏览器存储权限后重试。": "The safety state for this reset could not be saved. Check browser storage permissions and try again.", "上游结果未知。请保留当前弹窗并直接重试，系统会复用同一个幂等键和重置次数。": "The upstream outcome is unknown. Keep this dialog open and retry here so the same idempotency key and reset credit are reused.", "重置请求返回未知结果。请保留当前弹窗并直接重试。": "The reset request returned an unknown result. Keep this dialog open and retry here.",
    "重置已经完成，但最新用量或重置次数刷新失败；旧数据已清除，请点击“刷新用量与重置次数”重试。": "The reset completed, but the latest usage or reset credits could not be refreshed. Stale data was cleared; refresh usage and reset credits to try again.",
    "重新加载": "Reload",
    "全选": "Select all",
    "清空": "Clear",
    ...
    "关联团队": "Linked Teams", "默认团队": "Primary Team", "团队项目角色": "Team Project Role", "只读成员": "Viewer", "开发成员": "Developer", "项目维护者": "Maintainer", "仅团队负责人（兼容）": "Team leaders only (legacy)", "移除团队": "Remove Team", "其他团队": "Additional Teams", "主团队": "Primary Team",
